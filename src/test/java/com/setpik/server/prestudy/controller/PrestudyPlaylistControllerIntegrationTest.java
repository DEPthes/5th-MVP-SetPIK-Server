...
class PrestudyPlaylistControllerIntegrationTest {
	...
	@Test
	void deletesOwnedPrestudyPlaylist() throws Exception {
		mockMvc.perform(delete("/api/v1/prestudy-playlists/{id}", 701L)
				.header(HttpHeaders.AUTHORIZATION, bearerToken(1L)))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.code").value(1000))
			.andExpect(jsonPath("$.message").value("예습 플레이리스트가 삭제되었습니다."))
			.andExpect(jsonPath("$.result").doesNotExist());

		verify(prestudyPlaylistService).deletePrestudyPlaylist(1L, 701L);
	}
